...
function randomPlayerArb(): fc.Arbitrary<PlayerState> {
  return fc.record({
    ...
    insurance: fc.constantFrom('none' as const, 'basic' as const, 'full' as const),
    ...
    workedOutThisWeek: fc.integer({ min: 0, max: 8 }),
    burnout: fc.integer({ min: 0, max: 100 }),
    neglectWeeks: fc.integer({ min: 0, max: 20 }),
